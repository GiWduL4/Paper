...
general_path = Path(__file__).resolve().parents[3]  
  
# Add `folder1/basic_modules` to the system path  
basic_modules_path = str(general_path / "ATOMICS-python-modules/basic-modules")  
if basic_modules_path not in sys.path:  
    sys.path.append(basic_modules_path)  
...
def B0_pre(rho0):
    ...
    B0_prefactor = np.zeros((len(rho0), k_max), dtype = 'complex128')
...
def B2_pre(rho0):
    ...
    # Initialize the output as a 2D array (for each rho0, you get an array of size k_max)
    B2_prefactor = np.zeros((len(rho0), k_max), dtype = 'complex128')
